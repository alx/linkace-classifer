...
Demonstrates the classifier using the existing CSV data without needing live API connections
"""
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                links.append({
                    'url': row['url'],
                    'title': row['url'].split('/')[-1],  # Use last part of URL as title
                    'description': '',
                    'id': len(links) + 1,
...
def group_links_by_category(links: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
...
def demo_classification():
    ...
    for i, link in enumerate(input_links):
        ...
        try:
            ...
            if classifications:
                print(f"  ✅ Found {len(classifications)} high-confidence classifications:")
                ...
                print(f"  ⚠️  No classifications above threshold")
...
def demo_offline_classification(input_links: List[Dict[str, Any]], 
    ...
    for link in input_links:
        ...
        for list_id, classify_links in classify_lists_data.items():
            ...
            for classify_link in classify_links:
                classify_domain = classify_link['url'].split('/')[2] if len(classify_link['url'].split('/')) > 2 else classify_link['url']
                ...
                if domain == classify_domain:
                    ...
                    classifications.append({
                        'list_id': list_id,
                        'confidence': 0.75,
                        'reasoning': f'Similar domain: {domain} ~ {classify_domain}'
                    })
                    break
        ...
        if high_confidence:
            ...
            print(f"  → No high-confidence classifications")
