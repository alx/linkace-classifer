...
class OllamaClient:
    ...
    def _generate_classification_prompt(self, link_data: Dict[str, Any], 
                                      classify_lists_data: Dict[int, List[Dict[str, Any]]]) -> str:
        """
        ...
        """
        prompt = f"""You are a link classifier. Your task is to analyze a link and determine which classification lists it belongs to based on the content and context of existing links in those lists.
...
TASK:
Analyze the link to classify and determine which classification lists it belongs to. Consider:
...
For each classification list, provide a confidence score from 0.0 to 1.0 indicating how well the link fits that list.
...
Only include classifications where you have some confidence (>0.1). Be precise with confidence scores.
"""
    ...
    def classify_with_threshold(self, link_data: Dict[str, Any], 
        ...
        high_confidence_classifications = [
            classification for classification in all_classifications
            if classification["confidence"] >= threshold
        ]
        
        return high_confidence_classifications
    
    def get_best_classification(self, link_data: Dict[str, Any], 
                              classify_lists_data: Dict[int, List[Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
        """
    ...
    def batch_classify(self, links_data: List[Dict[str, Any]], 
                      classify_lists_data: Dict[int, List[Dict[str, Any]]],
                      threshold: float = 0.8) -> List[Dict[str, Any]]:
        """
        ...
        for i, link_data in enumerate(links_data):
            print(f"Classifying link {i+1}/{len(links_data)}: {link_data.get('url', 'N/A')}")
