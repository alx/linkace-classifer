...
try:
    ...
    ML_AVAILABLE = False
    print(
        "Warning: scikit-learn not available. Only rule-based classification will work."
    )
...
class URLClassifier:
    ...
    def load_url_lists(self, data_source, format_type="auto"):
        ...
        print(
            f"Loaded {len(self.categories)} categories with "
            f"{sum(len(urls) for urls in self.categories.values())} URLs"
        )
        ...
        for category in self.categories:
            ...
            if features["clean_domain"] in self.domain_patterns[category]:
                ...
                for domain_pattern in self.domain_patterns[category]:
                    if (
                        domain_pattern in features["domain"]
                        or features["domain"] in domain_pattern
                    ):
                        score += 5
                        break
    ...
    def classify(self, url: str, method="hybrid") -> Dict[str, any]:
        ...
        if self.ml_model and method in ["ml", "hybrid"]:
            try:
                ...
                results["ml"] = {"category": ml_category, "confidence": ml_confidence}
            except Exception as e:
        ...
        if method == "rule_based":
            ...
            if (
                results["ml"]
                and results["ml"]["category"]
                and results["ml"]["confidence"] > 0.7
            ):
                results["final_prediction"] = results["ml"]["category"]
                results["confidence"] = results["ml"]["confidence"]
            elif rule_category:
...
def main():
    ...
    for url in test_urls:
        ...
        print(f"URL: {url}")
        print(
            f"Rule-based: {result['rule_based']['category']} "
            f"({result['rule_based']['confidence']:.2f})"
        )
        if result["ml"]:
    ...
    for category, cat_stats in stats["categories"].items():
        print(
            f"  {category}: {cat_stats['url_count']} URLs, "
            f"{cat_stats['unique_domains']} domains"
        )
